...
const excelDateToMySQL = (serial) => {
  if (!serial || isNaN(serial)) return null;
  const jsDate = new Date((serial - 25569) * 86400 * 1000);
  const iso = jsDate.toISOString().split("T")[0]; // yyyy-mm-dd
  return iso;
};
...
export const importBillsData = async (req, res) => {
  ...
  try {
    ...
    const insertValues = rows.map((r) => [
      r.NO_BILL || null,
      r.REFERENCE || null,
      excelDateToMySQL(Number(r.SEND_DATE)) || null,
      r.SHIPPER_CODE || null,
    ...
    await connection.query(
      ...
      (
        NO_BILL, REFERENCE, SEND_DATE, SHIPPER_CODE, RECIPIENT_CODE,
